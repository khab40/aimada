...
prepare_bundle() {
  ...
  cp "${ROOT_DIR}/deployments/nebius-vm-compose.yml" "${bundle_dir}/docker-compose.yml"
  {
    ...
    write_env_line VITE_ENABLE_GOOGLE_AUTH "${VITE_ENABLE_GOOGLE_AUTH:-false}"
    write_env_line VITE_ENABLE_ADVANCED_ATTACK_CONTROLS "${VITE_ENABLE_ADVANCED_ATTACK_CONTROLS:-false}"
    write_env_line VITE_ENABLE_LEGACY_PAGES "${VITE_ENABLE_LEGACY_PAGES:-false}"
    ...
    write_env_line NEBIUS_JOB_IMAGE "${NEBIUS_JOB_IMAGE:-ghcr.io/khab40/ai-market-abuse-detection-arena-jobs:latest}"
