...
def test_spoofing_like_agent_places_and_cancels_labeled_wall() -> None:
    ...
    assert first[0].scenario_name == "spoofing-like"
...
def test_quote_stuffing_like_agent_emits_place_cancel_burst() -> None:
    agent = QuoteStuffingLikeAgent("ABUSER_03", scenario_id="scenario-3")
    ...
    assert {order.scenario_family for order in second} == {"quote_stuffing_like"}
...
def test_arena_engine_launches_scenario_with_labeled_events() -> None:
    ...
    assert state["active_scenario"]["scenario_name"] == "spoofing-like"
    assert "ABUSER_01" in state["active_agents"]
    assert any(event.get("scenario_family") == "spoofing_like" for event in state["events"])
